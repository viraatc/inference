...
def evaluate_math500(parsed: Optional[str], ground_truth: str) -> bool:
    ...
    prm800k_path = os.path.abspath(os.path.join(
        os.path.dirname(__file__), "submodules", "prm800k", "prm800k"))

    if not os.path.exists(prm800k_path):
        raise FileNotFoundError(
            f"PRM800K submodule not found at: {prm800k_path}")
    ...
    try:
        ...
        try:
            from submodules.prm800k.prm800k.grading.grader import grade_answer
        except ImportError as e:
            raise ImportError(f"Failed to import PRM800K grader: {e}")

    return grade_answer(given_answer=parsed, ground_truth=ground_truth)
