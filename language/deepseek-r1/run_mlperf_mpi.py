...
class DistributedOfflineSUT(BaseSUT):
    ...
    def get_results(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of result dictionaries with tok_model_output and tok_model_output_len
        """
        if self.rank != 0:
            return []
            ...
            if sample_id is not None and sample_id in self.results:
                ...
                if 'tokens' in result and result['tokens']:
                    ordered_results.append({
                        'tok_model_output': result['tokens'],
                        'tok_model_output_len': len(result['tokens'])
                    })
                else:
                ...
                if df is not None:
                    ...
                    if args.accuracy and df_output is not None:
                        ...
                        if mlperf_log_file.exists():
                            ...
                            # Get the directory of the saved output file for saving evaluated results
                            output_dir = Path(saved_file).parent

                            # Process dataframe and save evaluated results
                            df_evaluated, evaluated_file = process_and_save_dataframe(
                                df_output,
                                output_dir=output_dir,
                                base_filename=Path(saved_file).stem + "_evaluated.pkl"
                            )

                            # Print unified evaluation results
                            print_evaluation_results(df_evaluated, logger)

                    # Results are preserved for complete table at end
